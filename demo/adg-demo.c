...
main (gint argc, gchar **argv)
{
  ...
  g_signal_connect (widget, "expose-event", G_CALLBACK (piston_expose), canvas);
  ...
  g_signal_connect_swapped (widget, "clicked", G_CALLBACK (piston_to_ps), canvas);
  gtk_container_add (GTK_CONTAINER (button_box), widget);
...
static void
add_piston_path (AdgCanvas *canvas,
                 Piston    *piston)
{
  AdgEntity *entity;
...
static void
add_piston_dimensions (AdgCanvas *canvas,
                       Piston    *piston)
{
  double     A, B, C;
...
static void
piston_path_extern (AdgEntity *entity,
                    Piston    *piston)
{
  double   A, B, C;
  double   D1, D2, D3, D4, D5, D6, D7;
...
piston_to_ps (AdgCanvas *canvas)
{
  ...
  cairo_ps_surface_dsc_comment (surface, "%%Title: Automatic Drawing Generation (Adg) demo");
  cairo_ps_surface_dsc_comment (surface, "%%Copyright: Copyright (C) 2006 Fontana Nicola");
