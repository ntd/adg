...
cairo_bool_t
cpml_path_from_cairo		(CpmlPath		*path,
				 const cairo_path_t	*src,
				 cairo_t		*cr);
cairo_bool_t
cpml_path_from_cairo_explicit	(CpmlPath		*path,
				 const cairo_path_t	*src,
				 const CpmlPair		*org);
CpmlPath *
cpml_path_copy			(CpmlPath		*path,
				 const CpmlPath		*src);

cairo_bool_t
cpml_segment_from_path		(CpmlPath		*segment,
				 const CpmlPath		*path,
				 int			 index);

cairo_bool_t
cpml_primitive_from_path	(CpmlPath		*primitive,
				 const CpmlPath		*path,
				 int			 index);
cairo_bool_t
cpml_primitive_get_pair		(const CpmlPath		*primitive,
				 CpmlPair		*pair,
				 int			 index);
cairo_bool_t
