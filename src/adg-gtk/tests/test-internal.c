...
void
adg_gtk_test_init(int *p_argc, char **p_argv[])
{
    g_type_init();
...
const gpointer
adg_gtk_test_invalid_pointer(void)
{
...
static void
_adg_gtk_log_handler(const gchar *log_domain, GLogLevelFlags log_level,
                     const gchar *message, gpointer user_data)
{
}

static void
_adg_gtk_test_func(gconstpointer user_data)
{
    GCallback test_func;
    ...
    previous_handler = g_log_set_default_handler(_adg_gtk_log_handler, NULL);
...
void
adg_gtk_test_add_func(const char *testpath, GCallback test_func)
{
    g_test_add_data_func(testpath, (gconstpointer) test_func, _adg_gtk_test_func);
}
